...
#
#     for seq_id, features in parsed_data.items():
#         # Group all coordinates for a given protein product
#         cds_by_product = defaultdict(list)
#         for feature in features:
#             if feature['key'] == 'CDS':
#                 product = feature['qualifiers'].get('product', 'unknown_product')
#                 cds_by_product[product].extend(feature['coordinates'])
#                 if
#
#         # Process each distinct protein found in the sequence
#         for product, coords_list in cds_by_product.items():
#             # Sort coordinates by start position to ensure correct join() order
#             coords_list.sort(key=lambda x: int(x[0].strip('<>')))
#             # if codon_start is None: # complete CDS
#             #     codon_start = coords_list[0][0]
#             coord_strings = [f"{start}..{end}" for start, end in coords_list]
#             if len(coord_strings) == 1:
#                 formatted_coord = coord_strings[0]
#             else:
#                 # Format for spliced proteins (multiple exons)
#                 formatted_coord = f"join({','.join(coord_strings)})"
#
#             # Append a new dictionary for this protein instead of overwriting
#             formatted_output[seq_id].append({
#                 'product': product,
#                 'coordinate': formatted_coord
#             })
#
#     return dict(formatted_output)  # Convert back to a regular dict for the final output
