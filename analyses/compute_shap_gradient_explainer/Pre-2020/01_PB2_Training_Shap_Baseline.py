...
def main():
    ...
    data_path   = "/gpfs/fs7/grdi/genarcc/wp1/genomics_unit/IAV/part2/new/data/01_PB2"
    weight_path = "/gpfs/fs7/grdi/genarcc/wp1/genomics_unit/IAV/part2/new/models_weight/01_PB2"
    shap_out    = "/gpfs/fs7/grdi/genarcc/wp1/genomics_unit/IAV/part2/new/shap/01_PB2/"
    segment     = "01_PB2"
    unfold_size = 12
    ...
    logging.info("Training shape: %s", X_train.shape)
    ...
    for fold in range(10):
        ...
        train_indices, val_indices = list(splitter.split(X_train, y_train))[fold]
        X_CV_train, y_CV_train = resampling(X_train[train_indices], y_train[train_indices], n_downsamples=16000, n_upsamples=600,seed=0)
        
        X_val_chunks = np.array_split(X_train[val_indices], n_chunks)
        ...
        for chunk_id in range(n_chunks):
            start = time()
            logging.info("Explaining Chunk %d with shape %s", chunk_id, X_val_chunks[chunk_id].shape)
            X_expl = torch.tensor(X_val_chunks[chunk_id], device="cuda:0")
            raw_explanations = explainer.shap_values(X_expl)
            end = time()
            total_time = end - start
            logging.info("Done %d in %f seconds", chunk_id, total_time)
            np.save(f"{shap_out}/{segment}_training_shap_values_Fold_{fold}_chunk_{chunk_id}.npy", raw_explanations)
